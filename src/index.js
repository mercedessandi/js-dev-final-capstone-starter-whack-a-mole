...
const score = document.querySelector('#score'); // for the score element
const timerDisplay = document.querySelector('#timer'); // for the timer element.
...
function showAndHide(hole, delay){
  //the toggleVisibility function so that it adds the 'show' class.
  toggleVisibility(hole);
  
  const timeoutID = setTimeout(() => {
    // the toggleVisibility function so that it removes the 'show' class when the timer times out.
    toggleVisibility(hole);
    
    gameOver();
  }, delay); // changed the setTimeout delay to the one provided as a parameter
  return timeoutID;
}
